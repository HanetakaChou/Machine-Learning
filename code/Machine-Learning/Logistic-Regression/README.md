...
For the **binary classification**, we can learn from the **sigmoid function** that $\displaystyle \begin{cases} \mathop{\mathrm{\sigma}}(X \theta) \ge 0.5 & X \theta \ge 0 \\ \mathop{\mathrm{\sigma}}(X \theta) < 0.5 & X \theta < 0 \end{cases}$. (Technically, the threshold may NOT be 0.5, for example, when the **false positive** is considered.)  
...
We need to add one more constant feature column (of which the value remains 1) to calculate the **intercept term** of the **linear hyperplane**, just as what we do for the **linear regression**.  
...
For exmaple, we have the **circular decision boundary** $\theta_1 + \theta_2 x_1^2 + \theta_3 x_2^2 = 0$. We only need to consider the features $\displaystyle \begin{bmatrix} 1 & x_1^2 & x_2^2 \end{bmatrix}$, and the problem has been converted to the **linear decision boundary**.  
