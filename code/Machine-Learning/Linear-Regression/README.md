...
We assume the **linear model** $\displaystyle \hat{y} = \mathop{\mathrm{f_{\theta}}}(X) = X \theta$ of which $\displaystyle \hat{y}$ is the **prediction** which is estimated by our model and should be distinguished from the original **target** y, and $\displaystyle \theta$ is n **coeffiects** (or **weights**) which we would like to estimate.  
...
Usually, the **intercept term** (or **bias term**) is considered as the part of the X. We add one more constant feature column (of which the value remains 1) to implement that.  
...
A model is considered **good fit** if the model work well on the training data without the **noise**.  
